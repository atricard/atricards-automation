execute unless block ~ ~ ~ #empires:fluid run summon item ~ ~ ~ {Motion:[0.0,0.25,0.0],Item:{id:"minecraft:axolotl_spawn_egg",Count:1b,tag:{display:{Name:'{"text":"Conveyor Block","italic":false}'},CustomModelData:2100001,EntityTag:{id:"minecraft:area_effect_cloud",Duration:1,Tags:["conveyor","empires_machine","machine_init"]}}}}
...
#add direction tag
execute if entity @p[y_rotation=225..269] run tag @e[type=glow_item_frame,tag=conveyor_block,limit=1,sort=nearest] add convey_E
execute if entity @p[y_rotation=-180..44] run tag @e[type=glow_item_frame,tag=conveyor_block,limit=1,sort=nearest] add convey_S
...
execute if entity @p[y_rotation=135..224] run tag @e[type=glow_item_frame,tag=conveyor_block,limit=1,sort=nearest] add convey_N
