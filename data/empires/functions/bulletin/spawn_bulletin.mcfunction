...
data modify block ~ ~1 ~ Text1 set value '[{"text":"Building Bulletin ["},{"nbt":"CustomName","entity":"@e[type=armor_stand,tag=sigil,tag=complete,distance=..12,sort=nearest,limit=1]","interpret":true},{"text":"]"}]'
setblock ~ ~ ~ barrel{CustomName:'[{"text":""}]'} replace
data modify block ~ ~ ~ CustomName set from block ~ ~1 ~ Text1
setblock ~ ~1 ~ air replace
#
execute align xyz positioned ~0.5 ~0 ~0.5 run summon glow_item_frame ~ ~ ~ {Tags:["bulletin"],Silent:1b,Facing:1b,Invulnerable:1b,Invisible:1b,Fixed:1b,Item:{id:"minecraft:sheep_spawn_egg",Count:1b,tag:{CustomModelData:70101}}}
